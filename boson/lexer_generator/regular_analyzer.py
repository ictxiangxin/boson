...
interpreter: RegularInterpreter = RegularInterpreter()
...
class BosonRegularAnalyzer:
    def __init__(self, reference_nfa_mapping: Optional[Dict[str, LexicalNFA]] = None):
        self.__parser: RegularParser = RegularParser()
        self.__escape_character_mapping: Dict[str, Set[str]] = {
            'n': {'\n'},
            'r': {'\r'},
            't': {'\t'},
        ...
        self.__lexical_nfa: Optional[LexicalNFA] = None
        self.__reference_nfa_mapping: Dict[str, LexicalNFA] = {}
        if reference_nfa_mapping is not None:
            self.__reference_nfa_mapping: Dict[str, LexicalNFA] = reference_nfa_mapping
    ...
    def init_semantic(self):
        ...
        def _semantic_group(semantic_node: BosonSemanticsNode) -> BosonSemanticsNode:
            if len(semantic_node.children()) == 1:
                ...
                nfa: LexicalNFA = semantic_node[0].get_data()
                postfix: BosonSemanticsNode = semantic_node[1]
                if len(postfix.children()) > 0:
                    min_count: int = int(postfix[0].get_data())
                    ...
                    max_count: int = int(postfix[1].get_data())
        ...
        def _semantic_unicode(semantic_node: BosonSemanticsNode) -> BosonSemanticsNode:
            unicode_node: BosonSemanticsNode = BosonSemanticsNode()
        ...
        def _semantic_character(semantic_node: BosonSemanticsNode) -> BosonSemanticsNode:
            character: str = semantic_node[0].get_text()
            if len(character) > 1 and character[0] == '\\':
                escape_character: str = character[1]
        ...
        def _semantic_select(semantic_node: BosonSemanticsNode) -> BosonSemanticsNode:
            if (len(semantic_node.children())) == 1:
                reverse: bool = False
                select_list: BosonSemanticsNode = semantic_node[0]
            else:
                reverse: bool = True
                select_list: BosonSemanticsNode = semantic_node[1]
            select_character_set: Set[str] = set()
            for each_select in select_list.children():
                if len(each_select.children()) == 0:
                    character: str = each_select.get_text()
                    if character[0] == '\\':
                        escape_character: str = character[1]
                    ...
                    if start_unicode <= end_unicode:
                        character_range_set: Set[str] = set()
        ...
        def _semantic_reference(semantic_node: BosonSemanticsNode) -> BosonSemanticsNode:
            lexical_symbol: str = semantic_node[0].get_text()[1:-1]
