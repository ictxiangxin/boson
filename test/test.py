...
if __name__ == "__main__":
    ...
    bs_generate_python_code(slr_tables, reduce_code, literal)
    token_list, literal = bs_token_list("test/not_slr_grammar.txt")
    sentence_set, reduce_code = bs_grammar_analyzer(token_list)
    lr_tables = bs_lr_generate_table(sentence_set)
    bs_generate_python_code(lr_tables, reduce_code, literal)
    token_list, literal = bs_token_list("test/not_slr_grammar.txt")
    sentence_set, reduce_code = bs_grammar_analyzer(token_list)
    lalr_tables = bs_lalr_generate_table(sentence_set)
    bs_generate_python_code(lalr_tables, reduce_code, literal)
